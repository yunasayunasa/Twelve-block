...
class GameScene extends Phaser.Scene {
    ...
    // ★修正: hitBrick 関数
    hitBrick(ball, brick) {
         // 既に処理中の場合や、ボール・ブロックが無効な場合は抜ける
         if (this.isCheckingStageClear || !ball || !brick || !ball.active || !brick.active) {
             // console.log("hitBrick skipped - checking clear or inactive object");
             return;
         }
         ...
         if (hits > 0) {
             ...
             if (hits <= 0) {
                 ...
                 // ★ ステージクリアチェックを遅延実行
                 // チェック中フラグを立てる
                 this.isCheckingStageClear = true;
                 console.log("Scheduling stage clear check...");
                 this.time.delayedCall(1, () => { // 1ms 遅延 (ほぼ次のフレーム)
                     console.log("Executing delayed stage clear check...");
                     ...
                     this.isCheckingStageClear = false; // チェック完了、フラグ解除
...
const config = {
    ...
    physics: { default: 'arcade', arcade: { debug: true } }, // デバッグ表示継続

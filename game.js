...
class GameScene extends Phaser.Scene {
    ...
    preload(){}
    create(){ /* 省略 (変更なし、setupStage呼び出しはそのまま) */
        ...
        const iPW=this.gameWidth*PADDLE_WIDTH_RATIO; this.paddle=this.physics.add.image(this.gameWidth/2,this.gameHeight-PADDLE_Y_OFFSET,null).setDisplaySize(iPW,PADDLE_HEIGHT).setTint(0xffffff).setImmovable(true).setData('originalWidth',iPW);
        ...
        this.input.on('pointermove',(p)=>{ if(!this.isGameOver&&this.lives>0&&this.paddle&&!this.isStageClearing){ const tX = p.x; const hW = this.paddle.displayWidth / 2; const cX = Phaser.Math.Clamp(tX, hW, this.gameWidth - hW); this.paddle.x = cX; if (!this.isBallLaunched) { this.balls.getChildren().forEach(b => { if(b.active) b.x = cX; }); } } });
    ...
    handleBrickHit(brick, damage = 1) { /* 省略 (変更なし) */
        if (!brick || !brick.active || !brick.getData) return false;
        let currentHits = brick.getData('currentHits'); const maxHits = brick.getData('maxHits'); const originalTint = brick.getData('originalTint'); const isDurable = brick.getData('isDurable');
        currentHits -= damage; brick.setData('currentHits', currentHits);
        if (currentHits <= 0) { this.handleBrickDestruction(brick); return true; }
        else if (isDurable) { const darknessFactor = (maxHits - currentHits) * DURABLE_BRICK_HIT_DARKEN; brick.setTint(Phaser.Display.Color.ValueToColor(DURABLE_BRICK_COLOR).darken(darknessFactor).color); return false; }
        else { return false; }
     }
    ...
    handleBrickDestruction(brick) {
        ...
        // ★ ドロップ判定開始ログ
        console.log("[DEBUG] >> Starting Drop Checks");

        // バイシュラ判定
        const baisravaRoll = Phaser.Math.FloatBetween(0, 1);
        console.log(`[DEBUG] Baisrava Roll: ${baisravaRoll.toFixed(4)} (Threshold: ${BAISRAVA_DROP_RATE})`);
        if (baisravaRoll < BAISRAVA_DROP_RATE) {
            console.log("[DEBUG] !!! Baisrava Drop Triggered !!!");
            this.dropSpecificPowerUp(brickX, brickY, POWERUP_TYPES.BAISRAVA);
            console.log("[DEBUG] << Drop Checks End (Baisrava Dropped)");
            return true;
        }

        // 通常アイテム判定
        const normalRoll = Phaser.Math.FloatBetween(0, 1);
        console.log(`[DEBUG] Normal Drop Roll: ${normalRoll.toFixed(4)} (Threshold: ${POWERUP_DROP_RATE})`);
        if (normalRoll < POWERUP_DROP_RATE) {
            console.log("[DEBUG] !!! Normal Drop Triggered !!!");
            this.dropPowerUp(brickX, brickY); // 通常ドロップ関数呼び出し
        } else {
            console.log("[DEBUG] Normal Drop Check: No Drop");
        }

        console.log("[DEBUG] << Drop Checks End");
        return false;
    }
...
class UIScene extends Phaser.Scene {
    ...
    create() { /* 省略 (変更なし) */
        ...
        this.events.on('shutdown',()=>{ this.unregisterGameEventListeners(); });
    }
    ...
    activateVajraUIDisplay(iV,mV){ if(this.vajraGaugeText){ this.vajraGaugeText.setText(`奥義: ${iV}/${mV}`).setVisible(true); this.updateDropPoolPosition(); } }
    updateVajraGaugeDisplay(cV){ if(this.vajraGaugeText&&this.vajraGaugeText.visible){ this.vajraGaugeText.setText(`奥義: ${cV}/${VAJRA_GAUGE_MAX}`); this.updateDropPoolPosition(); } }

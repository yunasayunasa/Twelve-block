...
const MAKIRA_ATTACK_INTERVAL = 1000; // マキラ頻度調整済
const MAKIRA_BEAM_SPEED = 400;
const MAKIRA_BEAM_WIDTH = 10;
const MAKIRA_BEAM_HEIGHT = 15;
const MAKIRA_BEAM_COLOR = 0xff0000; // 赤色
const MAKIRA_FAMILIAR_OFFSET = 40;
const MAKIRA_FAMILIAR_SIZE = 10;
const MAKIRA_FAMILIAR_COLOR = 0x00ced1;
const MAKORA_PADDLE_GAP_RATIO = 0.05; // 隙間

// --- BootScene ---
class BootScene extends Phaser.Scene {
    constructor() { super('BootScene'); }
    preload() { console.log("B:Preload"); }
    create() { console.log("B:Create"); this.scene.start('TitleScene'); }
}
...
class TitleScene extends Phaser.Scene {
    ...
    create() {
        const w = this.scale.width, h = this.scale.height; this.cameras.main.setBackgroundColor('#222');
        ...
        const bs = { fontSize: '32px', fill: '#fff', backgroundColor: '#555', padding: { x: 20, y: 10 } }; const bhs = { fill: '#ff0' };
        const nb = this.add.text(w / 2, h * 0.5, '通常', bs).setOrigin(0.5).setInteractive({ useHandCursor: true }).on('pointerover', () => nb.setStyle(bhs)).on('pointerout', () => nb.setStyle(bs)).on('pointerdown', () => { console.log("通常"); this.scene.start('GameScene', { mode: GAME_MODE.NORMAL }); this.scene.launch('UIScene'); });
        const asb = this.add.text(w / 2, h * 0.7, '全員', bs).setOrigin(0.5).setInteractive({ useHandCursor: true }).on('pointerover', () => asb.setStyle(bhs)).on('pointerout', () => asb.setStyle(bs)).on('pointerdown', () => { console.log("全員"); this.scene.start('GameScene', { mode: GAME_MODE.ALL_STARS }); this.scene.launch('UIScene'); });
    }
}
...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.time.delayedCall(50, () => { if (this.scene.isActive('UIScene')) { this.events.emit('updateLives', this.lives); this.events.emit('updateScore', this.score); this.events.emit('updateStage', this.currentStage); } else console.warn("UI not active"); });
        this.physics.world.setBoundsCollision(true, true, true, false);
        this.physics.world.on('worldbounds', this.handleWorldBounds, this);
        const ipw = this.gameWidth * PADDLE_WIDTH_RATIO;
        this.paddle = this.physics.add.image(this.gameWidth / 2, this.gameHeight - PADDLE_Y_OFFSET, null).setDisplaySize(ipw, PADDLE_HEIGHT).setTint(0xffffff).setImmovable(true).setData('originalWidth', ipw);
        ...
        this.makoraBullets = this.physics.add.group(); // マコラ用だが現状使わない
        this.setColliders();
        this.physics.add.overlap(this.paddle, this.powerUps, this.collectPowerUp, null, this);
        // ★ マコラ有効時もパドル2とOverlap設定を追加 ★
        this.physics.add.overlap(this.paddle2, this.powerUps, this.collectPowerUp, (paddle, powerUp) => {
             // paddle2 が存在し、アクティブな場合のみ実行
            return this.paddle2 && this.paddle2.active;
        }, this);

        this.input.on('pointermove', (p) => {
            if (!this.isGameOver && this.lives > 0 && this.paddle && !this.isStageClearing) {
                const targetX = p.x;
                if (this.isMakoraActive && this.paddle2?.active) {
                    const gap = this.gameWidth * MAKORA_PADDLE_GAP_RATIO;
                    const paddleWidth = this.paddle.getData('originalWidth') || this.gameWidth * PADDLE_WIDTH_RATIO; // 安全のためデフォルト値
                    const halfWidth = paddleWidth / 2;
                    const minX = halfWidth + gap / 2; // 左パドルの左端が画面左端に来るX座標
                    const maxX = this.gameWidth - (halfWidth + gap / 2); // 右パドルの右端が画面右端に来るX座標
                    const clampedTargetX = Phaser.Math.Clamp(targetX, minX, maxX); // パドル群の中心を制限

                    this.paddle.x = clampedTargetX - gap / 2 - halfWidth;
                    this.paddle2.x = clampedTargetX + gap / 2 + halfWidth;

                    if (!this.isBallLaunched) {
                        this.balls.getChildren().forEach(b => { if (b.active) b.x = clampedTargetX; }); // ボールは中央追従
                    }
                } else {
                    const hw = this.paddle.displayWidth / 2;
                    const ctx = Phaser.Math.Clamp(targetX, hw, this.gameWidth - hw);
                    this.paddle.x = ctx;
                    if (!this.isBallLaunched) { this.balls.getChildren().forEach(b => { if (b.active) b.x = ctx; }); }
                }
            }
        });
    ...
    update() {
        if (this.isGameOver || this.isStageClearing || this.lives <= 0) return; let ac = 0; let sa = [];
        this.balls.getChildren().forEach(b => { if (b.active) { ac++; if (this.isBallLaunched && !this.isStageClearing && b.y > this.gameHeight + b.displayHeight) { if (b.getData('isAnilaActive')) this.triggerAnilaBounce(b); else { b.setActive(false).setVisible(false); if (b.body) b.body.enable = false; } } if (b.getData('isSindara')) { sa.push(b); if (b.getData('isAttracting')) this.updateSindaraAttraction(b); } if (b.body && this.isBallLaunched) { const min = NORMAL_BALL_SPEED * 0.1, max = NORMAL_BALL_SPEED * 5, sp = b.body.velocity.length(); if (sp < min && sp > 0) b.body.velocity.normalize().scale(min); else if (sp > max) b.body.velocity.normalize().scale(max); } } });
        if (sa.length === 1 && this.balls.getTotalUsed() > 1) { const r = sa[0]; if (r.getData('isSindara')) { this.deactivateSindara([r]); this.updateBallTint(r); } }
        if (ac === 0 && this.isBallLaunched && !this.isStageClearing && this.lives > 0) this.loseLife();
        ...
        if (this.isMakiraActive && this.paddle && this.familiars) { const px = this.paddle.x; const py = this.paddle.y - PADDLE_HEIGHT / 2 - MAKIRA_FAMILIAR_SIZE; const c = this.familiars.getChildren(); if (c.length >= 1 && c[0].active) c[0].setPosition(px - MAKIRA_FAMILIAR_OFFSET, py); if (c.length >= 2 && c[1].active) c[1].setPosition(px + MAKIRA_FAMILIAR_OFFSET, py); }
        if (this.makiraBeams) { this.makiraBeams.children.each(bm => { if (bm.active && bm.y < -MAKIRA_BEAM_HEIGHT) bm.destroy(); }); }
        if (this.makoraBullets) { this.makoraBullets.children.each(bu => { if (bu.active && bu.y < -MAKORA_BULLET_SIZE * 2) bu.destroy(); }); }
    }

    setColliders() {
        if (this.ballPaddleCollider) this.ballPaddleCollider.destroy(); if (this.ballBrickCollider) this.ballBrickCollider.destroy(); if (this.ballBrickOverlap) this.ballBrickOverlap.destroy(); if (this.ballBallCollider) this.ballBallCollider.destroy(); if (this.ballPaddle2Collider) this.ballPaddle2Collider.destroy(); this.ballPaddle2Collider = null;
        ...
        if (this.paddle2 && this.paddle2.active) { this.ballPaddle2Collider = this.physics.add.collider(this.paddle2, this.balls, this.hitPaddle, null, this); console.log("P2 collider set."); }
        this.ballBrickCollider = this.physics.add.collider(this.bricks, this.balls, this.hitBrick, (br, b) => { const ib = b.getData('isBikara'); const ip = b.getData('isPenetrating'); const im = b.getData('isSindara') && b.getData('isMerging'); if (ib || ip || im) return false; return true; }, this);
        this.ballBrickOverlap = this.physics.add.overlap(this.balls, this.bricks, this.handleBallBrickOverlap, (b, br) => b.getData('isPenetrating') || (b.getData('isSindara') && (b.getData('isAttracting') || b.getData('isMerging'))) || b.getData('isBikara'), this);
        ...
        if (this.makiraBeams && this.bricks) { this.physics.add.overlap(this.makiraBeams, this.bricks, this.hitBrickWithMakiraBeam, null, this); }
        // if (this.makoraBullets && this.bricks) { this.physics.add.overlap(this.makoraBullets, this.bricks, this.hitBrickWithMakoraBullet, null, this); } // マコラ弾は使わない
    ...
    hitBrickWithMakiraBeam(beam, brick) { console.log(">>> hitBrickWithMakiraBeam ENTER"); if (!beam || !brick || !beam.active || !brick.active || this.isStageClearing || this.isGameOver) { console.log(">>> hitBrickWithMakiraBeam aborted"); return; } console.log(">>> Makira beam hit brick at:", brick.x, brick.y); const brickX = brick.x, brickY = brick.y; try { console.log(">>> Destroying beam..."); beam.destroy(); console.log(">>> Beam destroyed."); console.log(">>> Disabling brick body..."); brick.disableBody(true, true); console.log(">>> Brick body disabled."); console.log(">>> Updating score..."); this.score += 10; this.events.emit('updateScore', this.score); console.log(">>> Score updated."); console.log(">>> Increasing Vajra gauge..."); this.increaseVajraGauge(); console.log(">>> Vajra gauge increased."); console.log(">>> Checking for powerup drop (delayed)..."); if (Phaser.Math.FloatBetween(0, 1) < POWERUP_DROP_RATE) { this.time.delayedCall(1, () => { console.log(">>> Dropping powerup from Makira hit"); this.dropPowerUp(brickX, brickY); }, [], this); } else { console.log(">>> No powerup drop."); } console.log(">>> Checking stage clear..."); if (!this.isStageClearing && this.bricks.countActive(true) === 0) { console.log(">>> Makira beam destroyed last brick!"); this.stageClear(); } console.log(">>> Stage clear check finished."); } catch(error) { console.error("!!!!!!!!!! ERROR in hitBrickWithMakiraBeam !!!!!!!!!!", error); if (beam && beam.active) { beam.setActive(false).setVisible(false); if(beam.body) beam.body.enable = false; } } console.log(">>> hitBrickWithMakiraBeam END"); }

    // ★★★ マコラ関連 (パドル分裂版 - 幅修正) ★★★
    activateMakora() {
        console.log(">>> Activating Makora (Paddle Split)!");
        if(this.isMakiraActive) this.deactivateMakira();
        this.deactivateVajra();

        if (!this.isMakoraActive) {
            this.isMakoraActive = true;
            const originalWidth = this.paddle.getData('originalWidth');
            if (!originalWidth) { console.error("Original paddle width not found!"); return;} // 安全策
            this.createSecondPaddle(); // paddle2 を元の幅で生成
            // パドル位置を初期調整
            if (this.paddle && this.paddle2?.active) { // paddle2 が正常に生成されたか確認
                 const gap = this.gameWidth * MAKORA_PADDLE_GAP_RATIO;
                 const halfWidth = originalWidth / 2;
                 const currentMidX = this.paddle.x + halfWidth; // 分裂前のパドル中心
                 const minX = halfWidth + gap/2;
                 const maxX = this.gameWidth - minX;
                 const targetX = Phaser.Math.Clamp(currentMidX, minX, maxX); // 画面内に収める

                 this.paddle.x = targetX - gap / 2 - halfWidth;
                 this.paddle2.x = targetX + gap / 2 + halfWidth;
                 // ボディサイズも明示的に設定（表示サイズと一致させる）
                 if (this.paddle.body) this.paddle.body.setSize(originalWidth, PADDLE_HEIGHT);
                 if (this.paddle2.body) this.paddle2.body.setSize(originalWidth, PADDLE_HEIGHT);
                 console.log(`>>> Initial paddle positions: P1=${this.paddle.x.toFixed(0)}, P2=${this.paddle2.x.toFixed(0)} Width=${originalWidth}`);
            } else { console.error(">>> Failed to position split paddles! P2 might be null or inactive."); }
            ...
            this.powerUpTimers[POWERUP_TYPES.MAKORA] = this.time.delayedCall(duration, () => { console.log(">>> Makora expired."); this.deactivateMakora(); this.powerUpTimers[POWERUP_TYPES.MAKORA] = null; }, [], this);
    ...
    deactivateMakora() {
        if (this.isMakoraActive) {
            ...
            if (this.paddle2) {
                if (this.ballPaddle2Collider) this.ballPaddle2Collider.destroy();
                this.paddle2.destroy(); this.paddle2 = null; this.ballPaddle2Collider = null;
    ...
    createSecondPaddle() {
        if (!this.paddle) {console.error("Cannot create paddle2, paddle1 missing!"); return;}
        if (this.paddle2 && this.paddle2.active) { console.warn(">>> Paddle2 already exists."); return; }
        const originalWidth = this.paddle.getData('originalWidth');
        if (!originalWidth) { console.error("Cannot get originalWidth!"); return; }
        const gap = this.gameWidth * MAKORA_PADDLE_GAP_RATIO;
        const newPaddleX = this.paddle.x + (originalWidth / 2) + (gap / 2) + (originalWidth / 2); // 仮の位置
        this.paddle2 = this.physics.add.image(newPaddleX, this.paddle.y, null)
            .setDisplaySize(originalWidth, PADDLE_HEIGHT) // 元の幅
            .setTint(0xcccccc)
            .setImmovable(true);
        if (this.paddle2.body) {
             this.paddle2.body.setSize(originalWidth, PADDLE_HEIGHT); // ボディサイズも
             this.paddle2.body.setAllowGravity(false);
        } else { console.error("Failed to create paddle2 body!"); if(this.paddle2) this.paddle2.destroy();}
        console.log(">>> Paddle 2 created with original width.");
    }
...
class UIScene extends Phaser.Scene {
    constructor() { super({ key: 'UIScene', active: false }); this.livesText=null; this.scoreText=null; this.stageText=null; this.vajraGaugeText = null; }
    create() { console.log("UIScene Create"); this.gameWidth=this.scale.width; const ts={fontSize:'24px',fill:'#fff'}; this.livesText=this.add.text(16,16,'ライフ:',ts); this.stageText=this.add.text(this.gameWidth/2,16,'ステージ:',ts).setOrigin(0.5,0); this.scoreText=this.add.text(this.gameWidth-16,16,'スコア:',ts).setOrigin(1,0);
        this.vajraGaugeText = this.add.text(16, this.scale.height - 30, '奥義: -/-', { fontSize: '20px', fill: '#fff' }).setVisible(false);
        try{const gs=this.scene.get('GameScene');if(gs&&gs.events){console.log("UIScene: GS found.");this.registerGameEventListeners(gs);}else{this.scene.get('GameScene').events.once('create',this.registerGameEventListeners,this);console.log("UIScene: Waiting GS create...");}}catch(e){console.error("UIScene: Error get GS.",e);}
        this.events.on('shutdown',()=>{console.log("UIScene Shutdown.");try{const gs=this.scene.get('GameScene');if(gs&&gs.events){gs.events.off('updateLives',this.updateLivesDisplay,this);gs.events.off('updateScore',this.updateScoreDisplay,this);gs.events.off('updateStage',this.updateStageDisplay,this);gs.events.off('create',this.registerGameEventListeners,this);gs.events.off('activateVajraUI',this.activateVajraUIDisplay,this);gs.events.off('updateVajraGauge',this.updateVajraGaugeDisplay,this); gs.events.off('deactivateVajraUI', this.deactivateVajraUIDisplay, this);}}catch(e){/*Ignore*/}}); console.log("UIScene Create End"); }
    registerGameEventListeners(gs) { console.log("UIScene: Registering listeners."); if(!gs||!gs.events){console.error("UIScene: Invalid GS.");return;} gs.events.off('updateLives',this.updateLivesDisplay,this);gs.events.off('updateScore',this.updateScoreDisplay,this);gs.events.off('updateStage',this.updateStageDisplay,this);gs.events.off('activateVajraUI',this.activateVajraUIDisplay,this);gs.events.off('updateVajraGauge',this.updateVajraGaugeDisplay,this); gs.events.off('deactivateVajraUI', this.deactivateVajraUIDisplay, this);
        gs.events.on('updateLives',this.updateLivesDisplay,this);gs.events.on('updateScore',this.updateScoreDisplay,this);gs.events.on('updateStage',this.updateStageDisplay,this);gs.events.on('activateVajraUI',this.activateVajraUIDisplay,this);gs.events.on('updateVajraGauge',this.updateVajraGaugeDisplay,this); gs.events.on('deactivateVajraUI', this.deactivateVajraUIDisplay, this);
        try{if(gs.hasOwnProperty('lives'))this.updateLivesDisplay(gs.lives);if(gs.hasOwnProperty('score'))this.updateScoreDisplay(gs.score);if(gs.hasOwnProperty('currentStage'))this.updateStageDisplay(gs.currentStage); console.log("UIScene: Initial state updated.");}catch(e){console.error("UIScene: Error initial update.",e);} }
    updateLivesDisplay(l){if(this.livesText)this.livesText.setText(`ライフ: ${l}`);}
    updateScoreDisplay(s){if(this.scoreText)this.scoreText.setText(`スコア: ${s}`);}
    updateStageDisplay(st){if(this.stageText)this.stageText.setText(`ステージ: ${st}`);}
    activateVajraUIDisplay(iv,mv){console.log(`UIScene: Activate Vajra UI (${iv}/${mv})`);if(this.vajraGaugeText){this.vajraGaugeText.setText(`奥義: ${iv}/${mv}`).setVisible(true);}}
    updateVajraGaugeDisplay(cv){console.log(`UIScene: Update Vajra Gauge (${cv}/${VAJRA_GAUGE_MAX})`);if(this.vajraGaugeText&&this.vajraGaugeText.visible){this.vajraGaugeText.setText(`奥義: ${cv}/${VAJRA_GAUGE_MAX}`);}}
    deactivateVajraUIDisplay(){console.log("UIScene: Deactivate Vajra UI"); if(this.vajraGaugeText){this.vajraGaugeText.setVisible(false);}}
}
